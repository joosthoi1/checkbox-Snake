...
for i in range(1000):
    ...
    if a == 20:
        ...
        master.update()
textin = StringVar()
txt = Entry(master, width=10, textvariable=textin)
txt.grid(row=19, column=20)
textin.set('0.125')

lbl = Label(master, text='Select speed\nnormal is 0.125\nlower is faster')
lbl.grid(row=20, sticky=W, column=20)
Button(master, text='Start', command=var_states1).grid(row=18, column=20)
